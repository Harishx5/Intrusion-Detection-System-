...
from threat_intel_detector import ThreatIntelDetector
...
SUPABASE_KEY = os.environ.get(
    "SUPABASE_ANON_KEY", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InNhZW9mdWd5c2NqZmdxcW5xb3drIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NTY3OTA1ODYsImV4cCI6MjA3MjM2NjU4Nn0.92yIPYoeF3zCMt6UbOTuBmVIWKMs9UI6Xd51Q4dORKE"
)
EDGE_FUNCTION_URL = f"{SUPABASE_URL}/functions/v1/ingest-traffic"
AGENT_API_KEY = os.environ.get(
    "AGENT_API_KEY", "Harishkanna@4 "
)
TRUSTED_IPS = ["192.168.1.5", "192.168.1.9", "172.64.149.246"]
...
def send_batch():
    ...
    # Threat Intelligence detection (known malicious IPs)
    if threat_detector and packets:
        all_alerts.extend(threat_detector.check(packets))
...
def main():
    ...
    response_manager = ResponseManager(
        supabase_url=SUPABASE_URL,
        api_key=AGENT_API_KEY,
        dry_run=False,  # Now in production mode
    ...
    # Add hardcoded trusted IPs for immediate noise reduction
    for ip in TRUSTED_IPS:
        suppression_engine._rules.append({
            "id": f"hardcoded-{ip}",
            "rule_type": "trusted_ip",
            "value": ip,
            "is_active": True
        })

    threat_detector = ThreatIntelDetector()
    print("[*] Loading Threat Intelligence feed (blocklist)...")
    threat_detector.load_feed()
