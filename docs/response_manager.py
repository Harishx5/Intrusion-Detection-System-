...
class ResponseManager:
    ...
    def block_ip(
        ...
        if not self.dry_run:
                ...
                if platform.system() == "Windows":
                    ...
                    subprocess.run(
                        ["netsh", "advfirewall", "firewall", "add", "rule", 
                         f"name=IDS_BLOCK_{ip}", "dir=in", "action=block", f"remoteip={ip}"],
                        check=True,
                        capture_output=True,
                    )
                    logger.info(f"Windows Firewall rule added for {ip}")
                else:
            ...
            method = "netsh" if platform.system() == "Windows" else "iptables"
            logger.info(f"[DRY RUN] Would block {ip} via {method}")
    ...
    def unblock_ip(self, ip: str, reason: str = "") -> bool:
        ...
        if not self.dry_run:
            try:
                if platform.system() == "Windows":
                    subprocess.run(
                        ["netsh", "advfirewall", "firewall", "delete", "rule", f"name=IDS_BLOCK_{ip}"],
                        check=True,
                        capture_output=True,
                    )
                else:
